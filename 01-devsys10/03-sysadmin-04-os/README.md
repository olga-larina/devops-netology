...
### Комментарий по ДЗ
В первом задании я бы ещё посоветовал добавить

[Unit]
After=network.target

Ведь процесс запускает в том числе веб-сервер, было бы хорошо иметь сеть уже сконфигурированной.
По второму заданию не совсем понял, зачем нужен process_cpu_seconds_total - Total user and system CPU time spent in seconds. Что даст общее время (не проценты) работы процессора в этих режимах.
В пятом задании всё верно, но напомню, что действие ulimit распространяется не на всю систему в целом, а на текущий процесс (обычно это какая-то оболочка, например, bash) и порождённые им процессы. P.S. Вы это и так упомянули в ответе на задание 7.
Задание 6 подразумевало прямой запуск /bin/sleep в новом namespace. Вы запустили сначала bash, поэтому PID sleep - не 1. Но, думаю, вы поняли как этот механизм работает.